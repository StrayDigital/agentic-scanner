...
# ----------------------------
# Page audit (with Ghost Code check)
# ----------------------------
def audit_page(url: str, timeout: int) -> PageAudit:
    ...
    # Ghost Code check (Headless/CSR detection)
    if score == 0 and text_len < 500:
        warnings.append(
            "⚠️ Headless Architecture Detected: This page relies heavily on JavaScript. While Google renders this, many AI Agents (which consume raw HTML to save compute) see a blank page. You are invisible to 'Fast' crawlers."
        )
...
def simulate_ai_response(
    ...
    if ghost_detected and health_score == 0:
        return (
            "I’m seeing an almost-empty page in raw HTML. That usually means your site renders content in the browser with JavaScript. "
            "Many AI systems use fast HTML-only crawlers—so they may see a blank page and skip you."
        )
...
if run:
    ...
    # Ghost detection (site-wide): if ANY audited page triggered the headless warning
    ghost_detected = any(
        any("Headless Architecture Detected" in w for w in a.warnings) for a in audits if a.ok_fetch
    )
    ...
    for a in audits:
        ...
        if org_obj and "sameAs" in org_obj:
            tier1_present, socials_only, links = authority_tier_from_sameas(org_obj.get("sameAs"))
    ...
    if ghost_detected and health_score == 0:
        st.warning("⚠️ Technical Risk: Client-Side Rendering — Your content appears hidden behind JavaScript for fast crawlers.")
    ...
    for a in audits:
        with st.expander(f"{a.final_url} — {a.score}/100", expanded=False):
            ...
            if a.raw_kb < 2.0:
                st.caption("Warning: Source code is unusually empty. Content is likely hidden behind JS.")
